...
def pad_packed_collate(batch):
    ...
    # elif data_tuple[0].ndim == 4:
    #     max_len, h, w, c = data_tuple[0].shape
    #     data_np = np.zeros((len(data_tuple), max_len, h, w, c))

    # data_np = np.zeros((len(data_tuple), *data_tuple[0].shape))
    # TODO remove hard-coded length
    data_np = np.zeros((len(data_tuple), 29, *data_tuple[0].shape[1:]))
    for idx in range(len(data_np)):
        data_np[idx][: data_tuple[idx].shape[0]] = data_tuple[idx]

    if data_tuple[0].ndim == 4:
        data_np = np.einsum("bthwc->bcthw", data_np)
