...
from video_transformers.utils.torch import (
    get_num_total_params,
...
class TimeDistributed(nn.Module):
    ...
    def __init__(
        self, backbone: video_transformers.backbones.base.Backbone, low_memory=False
    ):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.low_memory:
            ...
            x = x.contiguous().view(
                batch_size * num_timesteps, num_channels, height, width
            )  # x: (BxT)xCxHxW
...
class VideoModel(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        self.neck = neck
        self.head = head
        self.threechannel = nn.Conv2d(29, 3, 1)
    ...
    def to_onnx(
        ...
        video_transformers.deployment.onnx.export(
            self, quantize, opset_version, export_dir, export_filename
        )
