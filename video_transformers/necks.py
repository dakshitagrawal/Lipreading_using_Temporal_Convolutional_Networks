...
class PostionalEncoder(nn.Module):
    ...
    def __init__(
        self, num_features: int, dropout_p: float = 0.0, num_timesteps: int = 30
    ):
        ...
        for time_ind in range(self.num_timesteps):
            for feat_ind in range(0, self.num_features, 2):
                ...
                position_encodings[time_ind, feat_ind + 1] = torch.cos(
                    torch.tensor(cos_input)
                )
...
class TransformerNeck(BaseNeck):
    ...
    def __init__(
        ...
        self.positional_encoder = PostionalEncoder(
