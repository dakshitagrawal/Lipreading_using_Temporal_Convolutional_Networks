...
from video_transformers.utils.torch import (
    unfreeze_last_n_stages as unfreeze_last_n_stages_torch,
)
...
class TimmBackbone(Backbone):
    def __init__(
        ...
        **backbone_kwargs,
        ...
        backbone = timm.create_model(
            model_name, pretrained=pretrained, **backbone_kwargs
        )
        if backbone.pretrained_cfg["classifier"] == "head.fc":
        ...
        elif backbone.pretrained_cfg["classifier"] == (
            "head.l",
            "head_dist.l",
        ):  # levit
            ...
            raise NotImplementedError(
                f"Backbone not supported: {backbone.pretrained_cfg}"
            )
    ...
    def unfreeze_last_n_stages(self, n):
        stages = [stage for stage in self.model.stem.children()] + [
            stage for stage in self.model.stages.children()
        ]
